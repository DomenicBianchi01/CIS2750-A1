	...
	while (name != NULL) {
		...
		while (variableList != NULL) {
			...
			if (strcmp(variableList->method, name->key) == 0 && variableList->flag == 1) {
				...
				clearArray(temp, 1000);
				clearArray(temp2, 1000);
				clearArray(fullName, 1000);
...
VariableNode * findAllVariableDeclarations(Node * list) {
	...
	int scopeLevel = 0;
	char type[25];
	char methodName[1000];
	bool oneLine = false;

	clearArray(methodName, 1000);
	clearArray(type, 25);
...
void modifyWithFunctionPointers(Node * list, char structName[]) {
	...
	while (list != NULL) {
		...
		/*Search for type*/
		if ((strcmp(list->key, "int") == 0 || strcmp(list->key, "float") == 0 || strcmp(list->key, "double") == 0 || strcmp(list->key, "short") == 0 || strcmp(list->key, "long") == 0 || strcmp(list->key, "void") == 0 || strcmp(list->key, "char") == 0) && lookForType == true && inFunctionBody == false) {
		...
		if (lookForOpenBracket == true) {
			...
			if (strcmp(list->key, "(") == 0) {
				...
				/*printf("%s--%s\n", functionStart->key, functionStart->next->key);*/
