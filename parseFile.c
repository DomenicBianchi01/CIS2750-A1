	...
	while (fgets(lineString, 256, dataFile) != NULL) {
		...
		for (i = 0; i < strlen(lineString); i++) {
			...
			switch(lineString[i]) {
                ...
                /*If header*/
				if (lineString[i] == '.' && lineString[i+1] == 'h' && lineString[i+2] == '>') {
	...
	while (list != NULL) {
		...
		if (strcmp(list->key, "{") == 0) {
			...
			if (strlen(list->key) > 0) {
				...
				if (list->key[strlen(list->key)-1] == '>' || (list->key[strlen(list->key)-1] == ';' && isComment == false && forLoop == false) || endOfComment == true) {
